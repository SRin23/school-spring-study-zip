package kr.hs.study.beans;

public class TestBean {
	private int data1;
	private double data2;
	private String data3;

	public TestBean() {
		System.out.println("TestBean의 기본 생성자");
	}

	public TestBean(int data1) {
		System.out.println("int형 매개변수 1개인 생성자");
		this.data1 = data1;
		this.data2 = 11.11;
		this.data3 = "spring1";
	}

	//생성자 오버로딩
	public TestBean(double data2) {
		System.out.println("double형 매개변수 1개인 생성자");
		this.data1 = 20;
		this.data2 = data2;
		this.data3 = "spring2";
	}
	
	public TestBean(String data3) {
		System.out.println("String형 매개변수 1개인 생성자");
		this.data1 = 30;
		this.data2 = 33.33;
		this.data3 = data3;
	}
	
	//Spring에서는 생성자를 호출을 설정할때 type의 순서가 바뀌어도 호출이 가능하다
	public TestBean(int data1, double data2, String data3) {
		System.out.println("매개변수가 3개인 생성자");
		this.data1 = data1;
		this.data2 = data2;
		this.data3 = data3;
	}
	
	//값 출력 메소드
	public void dataPrint() {
		System.out.println("data1 : " + this.data1);
		System.out.println("data2 : " + this.data2);
		System.out.println("data3 : " + this.data3);
	}
}
